...
This guide assumes you have a UR5 arm mounted to a `Clearpath Robotics Husky UGV <https://clearpathrobotics.com/husky-unmanned-ground-vehicle-robot/>`_,
...
and `Ridgeback <https://clearpathrobotics.com/ridgeback-indoor-robot-platform/>`_.  Other arms, such as the UR3 are also supported.
...
Clearpath Robotics provides a dedicated manipulation package intended to make using the UR5 and UR3 arms easier on several platforms, such as the Warthog, Husky and Ridgeback.
...
You should see the model of the Husky with the UR5 arm mounted to it:

.. image:: images/ros/urdf.png
    :alt: URDF of the Husky UGV with UR5 arm

If your robot also has additional sensors and other payloads, you may instead simply include the ``husky_ur5_description.urdf.xacro`` file
...
.. code-block:: bash
    ...
    # if you included the UR5 description in a different file, change the path as needed
    export HUSKY_URDF_EXTRAS=$(catkin_find husky_ur_description urdf/husky_ur5_description.urdf.xacro --first-only)
